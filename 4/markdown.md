...
学習者が途中で躓いたり、もう一度見返す際のわかりやすさを求める場合、簡易的な索引として目次に重要なキーワードを挿入することで実現できます。

キーワードを目次に挿入する場合、わかりやすさのため以下の点に留意してください。
- キーワードは何回も登場するなど、重要なものに限定する
- 目次に記載するキーワードの数は目次が見づらくならない程度（1行で表示できる程度）に抑える
- キーワードが最初に解説された章や節に「解説しているキーワード」として書く

目次に重要なキーワードを記載する場合、以下の例を参考に必要な節の部分に記載してください。
...
1. トップページの実装
  1.1.  Djangoの全体像
  1.2.  Djangoの構成要素
  　キーワード： モデル、テンプレート、ビュー
2. ブログアプリの実装(前編）
  2.1. アプリケーションの実装
    キーワード：INSTALLED_APPS
  2.2. モデルの定義
  　キーワード：models.py、マイグレーション
```

### 章ごとに目次を書く場合
```
1. トップページの実装
  トップページの実装を通して、Django の全体像を掴みます。
  キーワード： モデル、テンプレート、ビュー
2. ブログアプリの実装(前編）
  ブログアプリを実際に作るために必要なDB設計を学び、Django上で実際に作成します。
  キーワード：INSTALLED_APPS、models.py、マイグレーション
```
